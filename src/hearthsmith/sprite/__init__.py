"""The avatar. A separate process from hearthsmithd — it needs a display, the daemon doesn't.
...
Layer model (all customization is data, no code):
  ...
  ~/.config/hearthsmith/avatar.yaml   which layer variant + palette per slot
