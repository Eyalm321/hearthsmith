"""hearthsmith-sprite: GTK3 always-on-top, click-through, transparent window that polls sprite.json and
...
from hearthsmith.sprite import pack as packmod
from hearthsmith.sprite import procedural
from hearthsmith.sprite.procedural import AVATAR_CFG

log = logging.getLogger("hearthsmith.sprite")
BUBBLE_W = 300
ALIVE_FILE = STATE_DIR / "sprite.alive"
# the shipped sheets live beside the package in a checkout (src/hearthsmith → assets/sheets)
SHEETS_DIR = Path(__file__).resolve().parents[3] / "assets/sheets"


def _bin(name: str) -> str:
    """The CLI from the same install: install.sh links it into ~/.local/bin, which the service
    unit's PATH may not have. The renderer runs on the system python (GTK), not the venv."""
...
class Sprite(Gtk.Window):
    ...
    def open_menu(self, ev) -> None:
        ...
        sheets = Gtk.Menu()
        for p in sorted(SHEETS_DIR.glob("*.png")):
        ...
        m.append(Gtk.SeparatorMenuItem())
        item("Hide", lambda: self._systemctl("stop", "hearthsmith-sprite.service"))
        m.show_all()
    ...
    def _switch_sheet(self, sheet: Path) -> None:
        ...
        subprocess.run([_bin("hearthsmith-sprite-slice"), str(sheet), str(out),
    ...
    def _forge(self, *args: str) -> None:
        import subprocess
        subprocess.Popen([_bin("hearthsmith"), *[a for a in args if a]])
    ...
    def open_prompt(self) -> None:
        ...
        css.load_from_data(b"""
            #hearthsmith-prompt { background: rgba(20,20,22,0.96); border: 2px solid #ff9a2e;
                            border-radius: 10px; padding: 6px 12px; }
            #hearthsmith-prompt entry { background: transparent; border: none; box-shadow: none;
                                  color: #f2f2f2; font-family: monospace; font-size: 15px;
                                  caret-color: #ff9a2e; }
            #hearthsmith-prompt label { color: #ff9a2e; font-size: 26px; margin-right: 12px;
                                  margin-left: 2px; }
        """)
        Gtk.StyleContext.add_provider_for_screen(w.get_screen(), css,
                                                 Gtk.STYLE_PROVIDER_PRIORITY_APPLICATION)
        box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL)
        box.set_name("hearthsmith-prompt")
    ...
    def _submit(self, text: str) -> None:
        ...
        def run():
            try:
                out = subprocess.run([_bin("hearthsmith"), "say", "--json", text],
    ...
    def _speak(self, text: str) -> None:
        ...
        def run():
            try:
                out = subprocess.run([_bin("hearthsmith"), "speak", text],
...
def main() -> None:
    ...
    _add_fonts()
    ap = argparse.ArgumentParser(prog="hearthsmith-sprite")
    ...
    ap.add_argument("--pack", type=Path, help="sprite pack dir (default ~/.config/hearthsmith/pack)")
