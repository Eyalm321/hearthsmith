...
Capture goes through the forge-windows GNOME extension (Wayland gives clients no way to read the
    ...
    try:
        out = subprocess.run(
            ["gdbus", "call", "--session", "--dest", "org.gnome.Shell",
             "--object-path", "/org/forge/Windows", "--method", "org.forge.Windows.Screenshot",
...
def verify(cfg: ComposeCfg, goal: str, rect: tuple[int, int, int, int] | None = None
    ...
    shot = capture(rect)
    if shot is None:
        return None, "no screenshot (forge-windows extension not loaded?)"
...
def _cli() -> None:
    """forge-look "<goal>" — take one look and say whether it is visibly done."""
    import sys
