"""forge-mcp: the store as an MCP server. Any harness (Claude Code, dsh, Codex, OpenClaw) is a
client. stdio transport; register with e.g. `claude mcp add forge -- forge-mcp`."""
...
@mcp.tool()
def forge_tasks_list(state: str = "open", project: str | None = None) -> str:
...
@mcp.tool()
def forge_tasks_add(title: str, due: str | None = None, project: str | None = None,
...
@mcp.tool()
def forge_tasks_done(task_id: str) -> str:
...
@mcp.tool()
def forge_tasks_block(task_id: str, reason: str = "") -> str:
...
@mcp.tool()
def forge_tasks_snooze(task_id: str, minutes: int = 120) -> str:
...
@mcp.tool()
def forge_nags_recent(n: int = 5) -> str:
